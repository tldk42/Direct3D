...
{
	G_Context.GetImmediateDeviceContext()->UpdateSubresource(mConstantBuffer.Get(), 0, nullptr, &mProjectionMatrix, 0, 0);
}

void CProjectionBuffer::Render()
{
	G_Context.GetImmediateDeviceContext()->VSSetConstantBuffers(2, 1, mConstantBuffer.GetAddressOf());
}
